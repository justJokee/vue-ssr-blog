...
export default {
  ...
  getArticles({ commit, state }, payload) {
    ...
    return api.get('/api/front/article/gets', params).then(data => {
      if (!payload.tag) {
  ...
  timeArticles({ commit, state }, payload) {
    return api.get('/api/front/article/search', payload).then(data => {
  ...
  getArticle({ commit, state }, payload) {
    return api
      ...
      .then(data => {
        ...
        if (data.length) {
          return api.get('/api/front/article/preAndNext', { date: data[0].date, cache: true }).then(data1 => {
  ...
  getHot({ state }) {
    return api.get('/api/front/article/hot', {}).then(data => {
  ...
  preAndNext({ commit, state }, payload) {
    return api.get('/api/front/article/preAndNext', payload).then(data => {
  ...
  loveArticle({ commit }, payload) {
    return api.patch('/api/front/article/love', payload)
  },
